...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    2) Launch 3+ parallel searches on the first action. Use broad-to-narrow strategy: start wide, then refine. When mapping the repo, note build entrypoints: **`package.json`**, **`pom.xml`**, **`build.gradle` / `build.gradle.kts`**, **`src/main/java`**, etc., so callers pick the right toolchains.
