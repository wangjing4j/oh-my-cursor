...
<Agent_Prompt>
  ...
  <Constraints>
    ...
    - Plan files (.oh-my-cursor/plans/*.md) are READ-ONLY.
  </Constraints>
  ...
  <Note_for_oh_my_cursor_Team>
    If the user arrived from **oh-my-cursor Team** mode with a checklist in `.oh-my-cursor/plans/team-tasklist.md`, prefer **one commit per completed requirement-level task**: a top-level deliverable (or a parent block once all nested checkboxes under it are done). Team CLI prompts request this by default. Split into more commits only when one requirement still mixes unrelated concerns.
  </Note_for_oh_my_cursor_Team>
