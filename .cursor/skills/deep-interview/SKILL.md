...
handoff: .oh-my-cursor/specs/deep-interview-{slug}.md
level: 3
---

<Cursor_oh_my_cursor>

This skill lives at `.cursor/skills/deep-interview/SKILL.md`. Cursor agents pick it up like other skills; natural-language triggers align with `<Use_When>` below.

**CLI (headless):** `oh-my-cursor interview [--quick|--standard|--deep] [--autoresearch] -- "<idea>"` runs the **analyst** role with this document inlined. Persist interview state and specs under **`.oh-my-cursor/`** (not legacy `.omc/`).

**vs Claude Code / upstream OMC:** Invocations like `Skill("oh-my-claudecode:omc-plan")`, `autopilot`, or `ralph` are **not** bundled here. After you have a spec, use **`oh-my-cursor team 'N:role "…"'`** with the spec path in the task, or chain **`oh-my-cursor agent planner` / `agent architect` / `agent critic`** manually. The **`omc autoresearch`** CLI is **not** in this package — for that flow use [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) or your own evaluator automation.

**Resume:** Ask for deep interview again or re-run `oh-my-cursor interview -- "<idea>"` and load `.oh-my-cursor/state/deep-interview-state.json` when present.
...
2. **Write to file**: `.oh-my-cursor/specs/deep-interview-{slug}.md`
...
- Use `Write` tool to save the final spec to `.oh-my-cursor/specs/`
...
- [ ] Spec file written to `.oh-my-cursor/specs/deep-interview-{slug}.md`
...
If interrupted, run `/deep-interview` again. The skill reads state from `.oh-my-cursor/state/deep-interview-state.json` and resumes from the last completed round.
...
/deep-interview "vague idea"
  ...
  → Spec written to .oh-my-cursor/specs/deep-interview-{slug}.md
  ...
  → /omc-plan --consensus --direct (spec as input, skip interview)
    ...
    → Consensus plan written to .oh-my-cursor/plans/
